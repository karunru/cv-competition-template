...
class ROCStarLoss(nn.Module):
    ...
    def forward(self, y_pred, _y_true):
        """
        ...
        """
        if self.last_whole_y_t is None:
            self.whole_y_pred = np.append(
                self.whole_y_pred, y_pred.clone().detach().cpu().numpy()
            )
            self.whole_y_t = np.append(
                self.whole_y_t, _y_true.clone().detach().cpu().numpy()
            )

            loss = F.binary_cross_entropy_with_logits(y_pred, 1.0 * _y_true)
        ...
        y_true = _y_true >= 0.50
        epoch_true = self.last_whole_y_t >= 0.50
        ...
        epoch_pos = self.last_whole_y_pred[epoch_true]
        epoch_neg = self.last_whole_y_pred[~epoch_true]
    ...
    def epoch_update_gamma(self, epoch=-1, delta=2):
        """
        ...
        pos = self.last_whole_y_pred[self.last_whole_y_t == 1]
        neg = self.last_whole_y_pred[
            self.last_whole_y_t == 0
        ...
        Lp = diff[
            diff > 0
        ]  # because we're taking positive diffs, we got pos and neg flipped.
        ln_Lp = Lp.shape[0] - 1

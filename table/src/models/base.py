...
from table.src.evaluation import calc_metric
from table.src.sampling import get_sampling
from table.src.utils import timer
...
class BaseModel(object):
    ...
    def post_process(
        ...
        if config["pre_process"]["xentropy"]:
            _y_train = y_train * 100
            _oof_preds = oof_preds * 100
            _test_preds = test_preds * 100
            _y_valid = y_valid * 100 if y_valid is not None else None
            _valid_preds = valid_preds * 100 if valid_preds is not None else None
        elif config["pre_process"]["tweedie"]:
            _y_train = 100 - y_train
            _oof_preds = 100 - oof_preds
            _test_preds = 100 - test_preds
            _y_valid = 100 - y_valid if y_valid is not None else None
            _valid_preds = 100 - valid_preds if valid_preds is not None else None
        else:
            _y_train = y_train
        ...
        if config["target"] == "Pawpularity":
            _oof_preds[_oof_preds < 0] = 0
            _oof_preds[_oof_preds > 100] = 100
            _test_preds[_test_preds < 0] = 0
            _test_preds[_test_preds > 0] = 100
            if _valid_preds is not None:
                _valid_preds[_valid_preds < 0] = 0
                _valid_preds[_valid_preds > 100] = 100
